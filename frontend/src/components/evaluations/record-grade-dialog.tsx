...
export function RecordGradeDialog({ open, onOpenChange }: RecordGradeDialogProps) {
  ...
  const { evaluations: mockRubrics, createGrade: createDemoGrade } = useAppData();
  ...
  const rubrics = isDemo ? mockRubrics : (apiRubrics ?? []);
  ...
  const handleSubmit = async () => {
    ...
    const rubric = rubrics.find((r) => r.id === Number(evaluationId));
    ...
    try {
      ...
      if (isDemo) {
        ...
        createDemoGrade({
          ...
          evaluationName: rubric?.name ?? "—",

          obtainedMarks: Number(obtainedMarks),

          totalMarks: rubric?.totalMarks ?? 100,

          isPublished: false,

          evaluatorName: user ? `Dr. ${user.firstName} ${user.lastName}` : "—",
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="button" onClick={() => void handleSubmit()} loading={!isDemo && createGrade.isPending}>{t("common.save")}</Button>
