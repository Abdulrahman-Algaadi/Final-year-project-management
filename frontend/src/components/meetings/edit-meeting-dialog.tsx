...
export function EditMeetingDialog({ meeting, open, onOpenChange }: EditMeetingDialogProps) {
  ...
  const { groups: mockGroups, updateMeeting: updateDemoMeeting } = useAppData();
  ...
  const groups = isDemo ? mockGroups : schedulableGroups;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isDemo) {
        ...
        updateDemoMeeting(meeting.id, {

          groupId: Number(form.groupId),

          groupName: group?.groupName ?? meeting.groupName,
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => void handleSave()} loading={!isDemo && update.isPending}>{t("common.save")}</Button>
