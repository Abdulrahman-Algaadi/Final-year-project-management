...
export function AdvisorDashboard() {
  ...
  const pendingSubmissions = (apiSubmissions ?? []).filter((s) => s.status === "Pending");
  ...
  const recentSubmissions = apiSubmissions ?? [];
  const pendingEvalsCount = dashboard?.evaluationsPending ?? 0;
  const assignedGroups = dashboard?.assignedProjects ?? schedulableGroups.length;
  const activeProjects = dashboard?.assignedProjects ?? schedulableGroups.filter((g) => g.projectTitle).length;
  const pendingCount = dashboard?.pendingSubmissions ?? pendingSubmissions.length;
  const upcomingCount = dashboard?.upcomingMeetings ?? upcomingMeetings.length;
